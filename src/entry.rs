...
impl<'a, V> Entry<'a, V> {
    #[inline]
    pub(crate) fn new(key: u32, int_map: &'a mut IntMap<V>) -> Self {
    ...
    fn indices(key: u32, int_map: &IntMap<V>) -> (usize, Option<usize>) {
        let cache_ix = int_map.calc_index(key);

        let vals = &int_map.cache[cache_ix];
        let vals_ix = { vals.iter() }
            .enumerate()
            .find_map(|(vals_ix, &(k, _))| (k == key).then_some(vals_ix));
